...
func TestEnvCredentialProvider_GetCredentials3(t *testing.T) {
	...
	err := os.Setenv("TMCLOUD_SDK_AK", MockAk)
	assert.Nil(t, err)
	err = os.Setenv("TMCLOUD_SDK_SK", MockSk)
	assert.Nil(t, err)
	p := BasicCredentialEnvProvider()
	credentials, err := p.GetCredentials()
	assert.Nil(t, err)
	...
	p = BasicCredentialEnvProvider()
	assert.Nil(t, err)
	err = os.Setenv("TMCLOUD_SDK_PROJECT_ID", MockProjectId)
	assert.Nil(t, err)
	credentials, err = p.GetCredentials()
	assert.Nil(t, err)
...
func TestEnvCredentialProvider_GetCredentials5(t *testing.T) {
	...
	assert.Nil(t, err)
	p := BasicCredentialEnvProvider()
	err = os.Setenv("TMCLOUD_SDK_IDP_ID", MockIdpId)
	assert.Nil(t, err)
	err = os.Setenv("TMCLOUD_SDK_ID_TOKEN_FILE", MockIdTokenFile)
	assert.Nil(t, err)
	err = os.Setenv("TMCLOUD_SDK_PROJECT_ID", MockProjectId)
	assert.Nil(t, err)
	credentials, err := p.GetCredentials()
...
func resetEnv() error {
	envs := []string{
		"TMCLOUD_SDK_AK",
		"TMCLOUD_SDK_SK",
		"TMCLOUD_SDK_PROJECT_ID",
		"TMCLOUD_SDK_DOMAIN_ID",
		"TMCLOUD_SDK_CREDENTIAL_TYPE",
		"TMCLOUD_SDK_IDP_ID",
		"TMCLOUD_SDK_ID_TOKEN_FILE",
	}
